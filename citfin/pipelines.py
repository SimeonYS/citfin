...
class CitfinPipeline:

    # Database setup
    conn = sqlite3.connect('citfin.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `citfin`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM citfin WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO citfin (date, title, link, content)"
